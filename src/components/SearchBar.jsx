...
export default function SearchBar({ watchlist = [], onAdd, limit = 25 }) {
  ...
  const search = async (q) => {
    ...
    try {
      ...
      setResults(list);
      setOpen(true); // always open so "no results" message can show
    } catch {
      setResults([]);
      setOpen(true);
    } finally {
    ...
    <div style={wrap}>
      ...
      <div style={inputBox}>
        ...
        <input
          ...
          onBlur={handleBlur}
          placeholder={atLimit ? `Watchlist full (${limit}/${limit})` : "Search by name or symbol…"}
          disabled={atLimit}
...
const noResults    = { padding: "12px 14px", fontFamily: "'IBM Plex Mono',monospace", fontSize: 12, color: "#6e7681" };