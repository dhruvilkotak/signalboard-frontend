...
export default function Trader({ watchlist = [] }) {
  ...
  const load = async () => {
    try {
      ...
      setError("Backend unavailable — make sure backend is running");
      console.error(e);
    }
  };
  ...
  if (error) {
    return (
      <div className="card" style={{ padding:24, textAlign:"center" }}>
        ...
        <div style={{ color:"var(--text2)", marginBottom:4 }}>{error}</div>
    ...
    <div>
      ...
      <div style={{ display:"flex", gap:12, marginBottom:16, flexWrap:"wrap" }}>
        {perf ? [
          ...
          <div className="hint mono">Loading account data...</div>
        )}
      </div>
        ...
        <div className="card" style={{ marginBottom:16 }}>
          ...
          <div style={{ height:8, background:"var(--bg3)", borderRadius:4, overflow:"hidden" }}>
            <div style={{
              ...
              width:`${Math.min(perf.progress_to_target_pct||0, 100)}%`,
      ...
      <div style={{ display:"flex", gap:8, marginBottom:16 }}>
        <button className="btn btn-primary" onClick={runAll} disabled={running}>
          {running ? "⟳ Running AI..." : "🤖 Run AI Auto-Trade"}
        </button>
      ...
      <div style={{ marginBottom:20 }}>
        ...
        {positions.length === 0 ? (
          <div className="hint">No open positions. Click "Run AI Auto-Trade" to start.</div>
        ) : positions.map(pos => (
      ...
      <div>
        ...
        {trades.length === 0 ? (
          <div className="hint">No trades executed yet.</div>
        ) : trades.map((t, i) => (
          <div key={i} className="card fade-in" style={{ marginBottom:8 }}>
            ...
            {t.signal_reason && (
              <div className="hint" style={{ marginTop:4, fontSize:11, fontStyle:"italic" }}>
                "{t.signal_reason}"
              </div>
            )}
          </div>
